import { Message, MessageEmbed, MessageReaction, ReactionEmoji, TextChannel, User } from 'discord.js'
import { IllyaClient } from '../Client'
import { CommandListener, ColorUtils, CommandContext, EmojiManager } from '../utils'

module.exports = class AnnounceCommand extends CommandListener {
  ...
  async run(message: Message, args: Array<string>, ctx: CommandContext) {
    ...
    if (msg.length > 2048) return ctx.quote('error', 'desculpe, mas o limite máximo de caracteres é de `2048` caracteres.')
    const embed = new MessageEmbed()
    embed.setColor(ColorUtils['pink'])
    embed.setFooter(`Anúncio enviado por: ${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
    embed.setTimestamp()
