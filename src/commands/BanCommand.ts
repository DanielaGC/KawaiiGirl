...
export default class BanCommand extends CommandListener {
  ...
  async run(message: Message, args: Array<string>, ctx: CommandContext) {
    ...
    if (!member)
      return ctx.quote(
        "error",
        "você precisa especificar um usuário para eu poder banir."
      );
    try {
      ...
      if (
        ...
        return ctx.quote(
          "error",
          "desculpe, você não pode banir alguém com um cargo igual ou maior que o seu."
        );
      if (!guild_member.bannable)
        return ctx.quote(
          "error",
          "desculpe, eu não posso banir este usuário, o cargo dele é maior que o meu."
        );
    } catch {
      return;
    }
    if (member.id === message.author.id)
      return ctx.quote(
        "error",
        "me desculpe, mas você não pode se auto banir."
      );
