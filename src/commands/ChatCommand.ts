...
export default class ChatCommand extends CommandListener {
  ...
  async run(message: Message, args: string[], ctx: CommandContext) {
    ...
    switch (args[0]) {
      case "on":
        {
          channel
            .edit(
              ...
              `Canal destrancado por ${message.author.tag}`
            )
            .then(() => {
              ctx.quote(
                "check_mark",
                "este canal foi destrancado com sucesso."
              );
            });
        }
        break;
      case "off":
        {
          channel
            .edit(
              ...
              `Canal trancado por ${message.author.tag}`
            )
            .then(() => {
      ...
      default: {
        ctx.quote(
          "error",
          "você precisa inserir `on` para liberar o canal ou `off` para trancar o canal."
        );
      }
    }
  }
