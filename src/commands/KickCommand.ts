...
module.exports = class KickCommand extends CommandListener {
  ...
  async run(message: Message, args: Array<string>, ctx: CommandContext) {
    ...
    if (!member) return ctx.quote('error', 'você precisa especificar um usuário para eu poder expulsar.')
    try {
      const guild_member = await message.guild.members.fetch(member.id)
      if (message.member.roles.highest.position <= guild_member.roles.highest.position) return ctx.quote('error', 'desculpe, você não pode expulsar alguém com um cargo igual ou maior que o seu.')
      if (!guild_member.kickable) return ctx.quote('error', 'desculpe, eu não posso expulsar este usuário, o cargo dele é maior que o meu.')
    } catch {
      return ctx.quote('error', 'este usuário não está servidor, por isso, eu não posso o expulsar.')
    }
    if (member.id === message.author.id) return ctx.quote('error', 'me desculpe, mas você não pode se auto expulsar.')
    ...
    message.guild.members.kick(member.id, reason).then(() => {
      ...
      if (modChannel) {
        ...
        embed.setAuthor('Usuário expulso', member.displayAvatarURL({ dynamic: true }))
