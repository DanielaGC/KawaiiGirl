  ...
  run(message: Message) {
    ...
    if (message.content.replace("!", "") === this.client.user.toString()) {
      return message.channel.send(
        `Olá ${message.author}, meu nome é \`${this.client.user.username}\` meu prefix é \`${process.env.PREFIX}\` espero que se divirta comigo (Não pense besteira).`
      );
    }
    ...
    if (userHas.length > 0)
      return ctx.quote(
        "error",
        `você não tem permissão para ${userHas
          .map((perms) => `\`${perms}\``)
          .join(", ")}, você não pode usar esse comando.`
      );
    if (botHas.length > 0)
      return ctx.quote(
        "error",
        `eu não tenho permissão para ${botHas
          .map((perms) => `\`${perms}\``)
          .join(", ")}, eu não posso executar esse comando.`
      );
    if (command.config.dev && !process.env.BOT_DEV.includes(message.author.id))
      return ctx.quote(
        "error",
        "apenas a minha criadora tem a permissão de executar este comando!"
      );
    try {
      ...
      embed.addField(
        `${EmojiManager.get("inbox_tray").mention} **Comando**`,
        `\`\`\`${command.config.name}\`\`\``
      );
      embed.addField(
        `${EmojiManager.get("outbox_tray").mention} **Erro**`,
        `\`\`\`${err}\`\`\``
      );
